...
Verify that our sequence log-probability computations match the `compute_transition_scores` method provided by the `transformers` library:
```{python}
#| eval: false
inputs = tokenizer("See you", return_tensors="pt")
# Note that if we specify do_sample=True, we also get Top-K with k=50, resulting in different scores than we
# calculate below as all other values are set to -Inf.
outputs = model.generate(**inputs, max_new_tokens=5, pad_token_id=tokenizer.eos_token_id, return_dict_in_generate=True, output_scores=True, output_logits=True)
...
# Compute transition scores only reports transition scores for generated tokens, i.e., skips input tokens
transition_probs = F.log_softmax(logits[:,(inputs["input_ids"].size(1)- 1):-1, :], dim=-1)
torch.gather(transition_probs, 2, outputs.sequences[:,inputs["input_ids"].size(1):,None]).squeeze(-1)
```
