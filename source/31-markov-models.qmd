...
It's also possible to instead use complete words as tokens, rather than characters. Modern tokenizers used in practice often use *subword* units, which are somewhere between characters and words in size. For example, here's a popular subword tokenizer implemented in the `tokenizers` library called *Byte-Pair Encoding* (BPE). BPE builds its vocabulary by iteratively merging the most common pairs of characters or subwords in the training text, which typically results in extremely common words, roots, and affixes being represented as single tokens.
...
$$
\begin{aligned}
    \mathbb{P}(T_k = t\mid T_{k-1} = t_{k-1}, T_{k-2} = t_{k-2}, \ldots , T_1 = t_1)= \mathbb{P}(T_k = t\mid T_{k-1} = t_{k-1}, T_{k-2} = t_{k-2}, \ldots , T_{k-n} = t_{k-n})\;.
\end{aligned}
$$
...
Well, "readability" here is a bit of a stretch. The reason this output looks so incomprehensible is that we set the context length `n` to zero, meaning that the model has no memory of previous tokens and is therefore just choosing random tokens according to their overall frequency in the training text. Let's try increasing the context length to 1:
